...
//    double TT_HARVEST_ADULTE;
    ...
    std::unique_ptr < Bunch > fruit;
    std::unique_ptr < MaleInflo > male;

    //     internals
    inflo::inflo_states statut;
    inflo::inflo_states pot_statut;
    inflo::inflo_sex sexe;
    inflo::inflo_sex avort;
...
public:
        ...
        male(new MaleInflo())
    {
        ...
        //         internals
        Internal(STATUS, &Inflo::statut);
        Internal(POT_STATUS, &Inflo::pot_statut);
        Internal(SEX, &Inflo::sexe);
        Internal(AVORT, &Inflo::avort);
        Internal(ESTIMATED_FLOWERING_DATE, &Inflo::estimated_flowering_date);
        Internal(ESTIMATED_HARVEST_DATE, &Inflo::estimated_harvest_date);
        ...
        Internal(FACTEUR_AGE_REGIMES, &Inflo::facteur_age_regimes);
        Internal(POT_FRUITS_NUMBER, &Inflo::pot_fruits_number);
        Internal(MALE_BIOMASS, &Inflo::male_biomass);
        Internal(THERMALTIMESINCEAPPEARANCE, &Inflo::thermalTimeSinceAppearance);
        Internal(INI_HARVEST_DATE, &Inflo::ini_harvest_date);

        //          externals
        External(DATE_PLUS_JEUNE_FEUILLE, &Inflo::date_plus_jeune_feuille);
        External(TREE_PRODUCTIONSPEED, &Inflo::tree_productionSpeed);
        External(PHYTOMER_RANK, &Inflo::phytomer_rank);
...
//        TT_HARVEST_ADULTE = parameters.get("TT_HARVEST_ADULTE");
        ...
        //        internals
        statut.add(inflo::UNKNOWN);
        statut.add(inflo::NON_ABLATED);
        pot_statut.add(inflo::UNKNOWN);
        sexe = inflo::UNKNOWN_SEX;
        ...
        facteur_age_regimes = max(1, INCREASE_TAILLE_REGIMES * (t  - date_plus_jeune_feuille) + FACTEUR_AGE_INI);
...
//        self.bunch.pot_fruits_number = self.bunch.facteur_age_regimes * GlobalVariables.MEAN_FRUIT_NUMBER_ADULTE
        ...
        if (phytomer_rank > parameters.get("ICsex_RANG_FIN")) {
           sexe = rd < parameters.get("INI_SEX_RATIO") ? inflo::FEMALE : inflo::MALE;
        }
...
//        estimated_harvest_date = pow((MINIMAL_PRODUCTION_SPEED / tree_productionSpeed), RATIO_DUREE_JEUNES_HARVEST) * (TT_HARVEST_ADULTE);
        inflo::inflo_states memstatut = statut;

        if(phytomer_rank < 0) {
            statut.add(inflo::INITIE);
            pot_statut.add(inflo::INITIE);
        } else {
            //TODO change Statut coz no removal
            statut.add( TT_corrige > estimated_flowering_date
                        ? (TT_corrige > estimated_harvest_date
                           ? inflo::RECOLTE
                           : inflo::FLORAISON_RECOLTE)
                        : inflo::APPARITION_FLORAISON );
        ...
        if (statut.is(inflo::FLORAISON_RECOLTE) && memstatut.is(inflo::APPARITION_FLORAISON) ) {
            ...
            fruit_model()->put(t, Bunch::OIL_BIOMASS, 0);
            fruit_model()->put(t, Bunch::NONOIL_BIOMASS, 0);
        }
    }
    ...
    void compute_biomass(double t) {
        if (!statut.is(inflo::RECOLTE)) {
            double fruit_oil_biomass = fruit->get< double >(t, Bunch::OIL_BIOMASS);
            double fruit_nonoil_biomass = fruit->get< double >(t, Bunch::NONOIL_BIOMASS);
            double peduncle_biomass = peduncle->get< double >(t, Peduncle::BIOMASS);
            male_biomass = male->get< double >(t, MaleInflo::BIOMASS);
            double fruit_oil_potential_biomass = fruit->get< double >(t, Bunch::OIL_POTENTIAL_BIOMASS);
            double fruit_nonoil_potential_biomass = fruit->get< double >(t, Bunch::NONOIL_POTENTIAL_BIOMASS);
            ...
            double fruit_oil_test_biomass = fruit->get< double >(t, Bunch::TEST_OIL_BIOMASS);
            double fruit_nonoil_test_biomass = fruit->get< double >(t, Bunch::TEST_NONOIL_BIOMASS);
...
//          phytomer.bunch.compute_flowering_date(GlobalVariables.PRODUCTION_SPEED_INITIAL)
//          phytomer.bunch.compute_harvest_date(GlobalVariables.PRODUCTION_SPEED_INITIAL)
...
//                  phytomers[phytomer.name].bunch.statut = "APPARITION_FLORAISON"
        ...
        if (sexe == inflo::FEMALE && avort != inflo::ABORTED) {
            peduncle->init_structure();
            fruit->init_structure();
        }
        if (sexe == inflo::MALE && avort != inflo::ABORTED)
    ...
    void compute(double t, bool /* update */)
    {
        ...
        if (avort != inflo::ABORTED and !statut.is(inflo::ABLATED)) {
            if (sexe == inflo::FEMALE) {
                peduncle->put(t, Peduncle::FR_RESTE, fr_reste);
                peduncle->put<inflo::inflo_states>(t, Peduncle::BUNCH_STATUS, statut);
                ...
                fruit->put(t, Bunch::FR_FRUITS, fr_fruits);
                fruit->put(t, Bunch::BUNCH_STATUS, statut);
                fruit->put(t, Bunch::ABLATION_STATUS, statut);//TODO Merge
                fruit->put(t, Bunch::FRUIT_NUMBER, fruit_number);
                fruit->put(t, Peduncle::TEFF, gain_TEff_jour);
                fruit->put(t, Bunch::TT_CORRIGE, TT_corrige);
                fruit->put(t, Bunch::PROD_SPEED, tree_productionSpeed);
                fruit->put(t, Bunch::TT_SINCE_APP, thermalTimeSinceAppearance);
                fruit->put(t, Bunch::ESTIM_FLO_DATE, estimated_flowering_date);
                fruit->put(t, Bunch::ESTIM_HARVEST_DATE, estimated_harvest_date);
                fruit->put(t, Bunch::POT_STATUS, pot_statut);
                (*fruit)(t);
            }
        ...
        //Demand
        if (statut.is(inflo::APPARITION_FLORAISON))
        ...
        demand =
                fruit->get< double >(t, Bunch::DEMAND) +
        ...
        if (phytomer_rank > IC_spikelet_RANG_FIN) {
            ...
            if (sexe == inflo::MALE)
                fruit_number = 0;
            if (avort == inflo::ABORTED)
                fruit_number = 0;
            if (statut.is(inflo::ABLATED))
                fruit_number = 0;
        }
